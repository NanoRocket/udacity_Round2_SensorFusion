...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            label_corners = tools.compute_box_corners(bbox.center_x,
                                                    bbox.center_y,
                                                    bbox.width,
                                                    bbox.length,
                                                    bbox.heading)
            ...
            for detection in detections:
                ...
                delta_x = bbox.center_x - x
                delta_y = bbox.center_y - y
                delta_z = bbox.center_z - z
                # dist = np.sqrt(delta_x**2+delta_y**2+delta_z**2)

                ## step 5 : compute the intersection over union (IOU) between label 
                # and detection bounding-box
                label_poly = Polygon(label_corners)
                det_poly = Polygon(det_corners)
                intersection = label_poly.intersection(det_poly)
                union = label_poly.union(det_poly)
                iou = intersection.area/union.area
  
                ## step 6 : if IOU exceeds min_iou threshold, store [iou,dist_x, dist_y, dist_z] 
                # in matches_lab_det and increase the TP count
                if iou > min_iou:
                    
                    matches_lab_det.append([iou, delta_x, delta_y, delta_z])
                    true_positives += 1
